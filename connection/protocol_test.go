...
func TestNewProtocolSelector(t *testing.T) {
	tests := []struct {
		...
		{
			name:             "named tunnel with edge tunnel",
			protocol:         AutoSelectFlag,
            needEdgeTunnel:   true,
			expectedProtocol: HTTP2,
		},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			selector, err := NewProtocolSelector(test.protocol, testAccountTag, test.tunnelTokenProvided, test.needPQ, fetcher.fetch(), ResolveTTL, &log, test.needEdgeTunnel)
			if test.wantErr {
...
func TestAutoProtocolSelectorRefresh(t *testing.T) {
	fetcher := dynamicMockFetcher{}
	selector, err := NewProtocolSelector(AutoSelectFlag, testAccountTag, false, false, fetcher.fetch(), testNoTTL, &log, false)
...
func TestHTTP2ProtocolSelectorRefresh(t *testing.T) {
	...
	selector, err := NewProtocolSelector(HTTP2.String(), testAccountTag, false, false, fetcher.fetch(), testNoTTL, &log, false)
	assert.NoError(t, err)
	assert.Equal(t, HTTP2, selector.Current())
...
func TestAutoProtocolSelectorNoRefreshWithToken(t *testing.T) {
	fetcher := dynamicMockFetcher{}
	selector, err := NewProtocolSelector(AutoSelectFlag, testAccountTag, true, false, fetcher.fetch(), testNoTTL, &log, false)
	assert.NoError(t, err)
	assert.Equal(t, QUIC, selector.Current())
