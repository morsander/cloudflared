...
// to connections.
func ResolveEdge(log *zerolog.Logger, region string, edgeIpVersion allregions.ConfigIPVersion, edgeTunnel string) (*Edge, error) {
	regions, err := allregions.ResolveEdge(log, region, edgeIpVersion, edgeTunnel)
	if err != nil {
		return new(Edge), err
	}
	return &Edge{
